...
import type { DatabaseStrategyOptions, Session, SessionStrategy, User } from "@/@types/session.ts"
import {
    __getSession,
    __createSession,
    __oauthCallback,
    __signIn,
    __revokeSession,
    __isProviderConnected,
    __getProviderTokens,
    __destroySession,
    __refreshSession,
    __revokeToken,
    __refreshUserInfo,
} from "@/session/stateful/index.ts"
...
export const createStatefulStrategy = <DefaultUser extends User = User>({
    ...
    return {
        refreshUserInfo,
        getSession: __getSession({ ctx: ctx as any, cookieConfig }),
        createSession: __createSession({ ctx: ctx as any, cookieConfig }),
        refreshSession: __refreshSession({ ctx: ctx as any, cookieConfig }),
        revokeSession: __revokeSession({ ctx: ctx as any, cookieConfig }),
        revokeToken: __revokeToken({ ctx: ctx as any, cookieConfig }),
        destroySession: __destroySession({ ctx: ctx as any, cookieConfig }),
        getProviderTokens: __getProviderTokens({ ctx: ctx as any, cookieConfig }),
        isProviderConnected: __isProviderConnected({ ctx: ctx as any, cookieConfig }),
        signIn: __signIn({ ctx: ctx as any, cookieConfig }),
        oauthCallback: __oauthCallback({ ctx: ctx as any, cookieConfig }),
    }
}
