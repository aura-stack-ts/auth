...
export const providers = (env: NodeJS.ProcessEnv) => {
    return Object.entries(builtInOAuthProviders).reduce((previous, [oauth, config]) => {
        const clientIdSuffix = `${oauth.toUpperCase()}_CLIENT_ID`
        const clientSecretSuffix = `${oauth.toUpperCase()}_CLIENT_SECRET`

        const clientIdInput = Boolean(
            env[`AURA_AUTH_${clientIdSuffix}`] ?? env[`AUTH_${clientIdSuffix}`] ?? env[`${clientIdSuffix}`]
        )
        ...
        return [
            ...previous,
            {
                ...
                redirectURI: `http://localhost:5174/auth/callback/${oauth}`,
