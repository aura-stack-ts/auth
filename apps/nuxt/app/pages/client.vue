...
const { session, status, isPending, signIn, signOut, signInCredentials, updateSession } = useAuthClient()
    ...
    <main class="w-11/12 min-h-container max-w-5xl mx-auto content-center">
        <section class="max-w-lg mx-auto space-y-8">
            <div class="text-center">
                <h1 class="text-2xl font-bold text-white">Aura Auth + Nuxt Client Components</h1>
                <p class="mt-2 text-base text-white/70 max-w-3xl">
                    Official Nuxt demo to showcase @aura-stack/auth authentication library with Client Side Rendering (CSR), for
                    Server-Side Rendering (SSR) visit
                    <NuxtLink class="text-white underline underline-offset-2" to="/server"> here </NuxtLink>
                </p>
            </div>
        </section>
