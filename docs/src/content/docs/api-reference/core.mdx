...
- Custom OAuth configuration implementations: create custom oauth providers doesn't supported natvely by aura auth, follow the `OAuthSecureConfig` type.

```ts lineNumbers
interface OAuthConfig<Profile extends object = {}> {
  id: string
  name: string
  authorizeURL: string
  accessToken: string
  ...
  scope: string
  responseType: string
  profile?: (profile: Profile) => Promise<OAuthUserProfile> | OAuthUserProfile
}
