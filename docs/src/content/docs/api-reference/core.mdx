...
```ts lineNumbers
type Secret = string | Uint8Array | CryptoKey | CryptoKeyPair | CryptoSecret
```
...
| `api`      | `AuthAPI`                          | API methods for programmatic interaction with authentication workflows (`getSession`, `signIn`, `signOut`, `updateSession`).                                                              |
| `jose`     | `JoseInstance`                     | Object that exports JOSE utilities for signing and encrypting JWTs and CSRF tokens.                                                                                                       |

##### `handlers`

The HTTP handlers are functions that control the logic and flow of the OAuth 2.0 authorization process. Currently, the `GET`, `POST`, `PATCH`, and `ALL` methods are exported. `ALL` is a unified handler that consolidates the method handlers into a single entry point. By default, Aura Auth sets the base path to `/auth`. For configuration details, refer to [basePath](/docs/api-reference/core#basepath).
...
Updates default session data from the initial OAuth profile data. Currently supports updates only for the `email`, `name`, and `image` fields, or any fields defined in the `identity.schema`. For broader claim support programmatically, use the `api.updateSession` function.
...
Object that exposes API functions for retrieving sessions and signing out on the server without calling mounted HTTP endpoints, improving resource access in trusted environments like Server Actions or loaders.

```ts lineNumbers
type AuthAPI = {
  getSession: (req: Request) => Promise<Session | null>
  signIn: (providerId: string, options?: SignInOptions) => Promise<Response>
  signOut: (options?: SignOutOptions) => Promise<Response>
  updateSession: (req: Request, data: Parital<User>) => Promise<Session | null>
}
```

- `getSession`: retrieves the session data securely via headers without hitting the endpoint.
- `signIn`: initiates an OAuth sign-in flow programmatically on the server.
- `signOut`: invalidates the active session programmatically on the server.
- `updateSession`: updates session data fields natively via headers without HTTP calls.
...
### `createAuthClient(config: ClientConfig)` [#create-auth-client]

Creates an authentication client instance enabling applications to interact with Aura Auth endpoints across the network through a dedicated client interface.
...
It exposes typing for built-in client functions to handle authentication seamlessly:

- `getSession`: fetches the current session asynchronously.
- `signIn`: triggers the OAuth flow by redirecting the user to `signIn/:provider`.
- `signOut`: hits the `/signOut` route appending the required CSRF tokens.
- `updateSession`: sends updated values dynamically to modify the user's active session.
