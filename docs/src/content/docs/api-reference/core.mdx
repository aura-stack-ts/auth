...
The HTTP handlers are functions that control the logic and flow of the OAuth 2.0 authorization process. Currently, the `GET`, `POST`, and `ALL` methods are exported. `ALL` is a unified handler that consolidates GET and POST into a single entry point. By default, Aura Auth sets the base path to `/auth`. For configuration details, refer to [basePath](/docs/api-reference/core#basepath).
