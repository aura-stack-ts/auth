...
Create a catch-all route in `app/api/auth/[...aura]/route.ts` to handle all authentication endpoints dynamically. The `/api/auth` path should match with the App Rouetr Structure and `basePath` defined in your Auth configuration.
