...
| ----------------------- | --------- | ------------------------------------------------------------------------------------------ |
| `BASE_URL`              | `string`  | Base URL of the application used to resolve relative URLs for OAuth redirects and cookies. |
| `TRUSTED_PROXY_HEADERS` | `boolean` | Enables support for trusted proxy headers when set to one of: `1`, `true`, `yes`, `on`.    |
...
```bash title=".env"
# Base URL of the application is mounted at
AURA_AUTH_BASE_URL="https://app.example.com"

# Enable support for trusted proxy headers
AURA_AUTH_TRUSTED_PROXY_HEADERS=true
