---
title: Google
description: Add Google authorization provider to Aura Auth for authentication and authorization
---
...
The first step is to create and register an OAuth App on the Google Client Credentials to obtain access to the user's resources.

1. Navigate to your Google Cloud Account and go to **APIs and Services > Credentials > OAuth Client IDs**.
2. Click **Create Credentials**.
3. Select **Application Type** and choose **Web application** as the application type.
4. Fill in the "Name".
5. Add the **Authorized JavaScript origins** as `http://localhost:3000`.
   - _(Make sure to replace `localhost:3000` with your production domain when deploying)_
6. Set the **Redirect URL** to `http://localhost:3000/auth/callback/google`.
   - _(Make sure to replace `localhost:3000` with your production domain when deploying)_
7. Click **Register application**.
8. Ensure you copy the **Client ID** and click **Generate a new client secret**.
...
Now, you must configure the environment variables required by Aura Auth, including the Google credentials and the encryption secrets.
...
# Google Credentials
AURA_AUTH_GOOGLE_CLIENT_ID="your_google_client_id"
AURA_AUTH_GOOGLE_CLIENT_SECRET="your_google_client_secret"
```
...
```ts title="auth.ts" lineNumbers
import { createAuth } from "@aura-stack/auth"
import { google } from "@aura-stack/auth/oauth/google"
...
## Sign In to Google (Client & Server)
...
`http://localhost:3000/auth/signIn/google`
...
export const serverSignIn = async () => {
  const response = await api.signIn("google", {
...
console.log(session?.user) // The authenticated Google user profile
```
...
- [Google - Client Credentials](https://console.cloud.google.com/auth/clients)
