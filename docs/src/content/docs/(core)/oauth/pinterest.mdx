---
title: Pinterest
description: Add Pinterest authorization provider to Aura Auth for authentication and authorization
---

## What you'll build

Through this quick start guide you are going to learn and understand the basics and how to set up the `Pinterest` provider for Aura Auth.
...
- [Sign In to Pinterest (Client & Server)](#sign-in-to-pinterest-client--server)
- [Resources](#resources)
...
The first step is to create and register an OAuth App on the Pinterest developer console to obtain access to the user's resources.

1. Navigate to the **Pinterest Developers** portal (`developers.pinterest.com/apps`).
2. Click **Create app**.
3. Provide a name and description for your app.
4. Under the "Platforms" section, select **Web** and enter your domain information.
5. In the **Redirect URIs** field, enter `http://localhost:3000/auth/callback/pinterest`.
   - _(Make sure to replace `localhost:3000` with your production domain when deploying)_
6. Add the relevant app scopes.
7. Save your settings. You can now copy your **App ID** (Client ID) and **App secret** (Client Secret).
...
Now, you must configure the environment variables required by Aura Auth, including the Pinterest credentials and the encryption secrets.
...
AURA_AUTH_PINTEREST_CLIENT_ID="your_pinterest_client_id"
AURA_AUTH_PINTEREST_CLIENT_SECRET="your_pinterest_client_secret"
```
...
## Sign In to Pinterest (Client & Server)
...
`http://localhost:3000/auth/signIn/pinterest`
...
const triggerSignIn = async () => {
  await authClient.signIn("pinterest", {
...
export const serverSignIn = async () => {
  const response = await api.signIn("pinterest", {
...
- [Pinterest - Authorizing OAuth Apps](https://developers.pinterest.com/docs/getting-started/authentication/)
