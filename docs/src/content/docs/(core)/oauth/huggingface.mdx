---
title: Hugging Face
description: Add Hugging Face authorization provider to Aura Auth for authentication and authorization
---
...
The first step is to create and register an OAuth App on the Hugging Face Connected Apps to obtain access to the user's resources.
...
4. Select **openid**, **profile**, and **email** Account's scopes for the application.
5. Set the **Redirect URLs** to `http://localhost:3000/auth/callback/huggingface`.
...
Now, you must configure the environment variables required by Aura Auth, including the Hugging Face credentials and the encryption secrets.
...
AURA_AUTH_HUGGINGFACE_CLIENT_ID="your_huggingface_client_id"
AURA_AUTH_HUGGINGFACE_CLIENT_SECRET="your_huggingface_client_secret"
```
...
```ts title="auth.ts" lineNumbers
import { createAuth } from "@aura-stack/auth"
import { huggingface } from "@aura-stack/auth/oauth/huggingface"
...
## Sign In to Hugging Face (Client & Server)
...
`http://localhost:3000/auth/signIn/huggingface`
...
const triggerSignIn = async () => {
  await authClient.signIn("huggingface", {
...
console.log(session?.user) // The authenticated Hugging Face user profile
```
...
- [Hugging Face - Create an OAuth App](https://huggingface.co/settings/applications/new)
- [Hugging Face - Sign in with Hugging Face](https://huggingface.co/docs/hub/en/oauth)
- [Hugging Face - OpenID Metadata](https://huggingface.co/.well-known/openid-configuration)
- [Hugging Face - Open API Metadata](https://huggingface.co/.well-known/openapi.json)
