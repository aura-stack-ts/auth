...
Defines the session behavior and management strategy. How user sessions are stored and maintained across requests. Aura Auth currently supports the `jwt` (stateless) strategy.
...
For the full list of `jwt` (`mode`, `issuer`, `audience`, etc.) and `maxAge`, `maxDuration`, `expirationStrategy` options and guidance on choosing between them, see [Session Strategies](/docs/configuration/session-strategies).
