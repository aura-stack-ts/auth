...
Aura Auth also supports `CryptoKey` and `CryptoKeyPair` objects for environments that support the Web Crypto API. This option allows for more secure key management and usage of asymmetric keys for signing and encryption related to the `jwt.mode` option. But requires additional setup to their correct generation and usage.
