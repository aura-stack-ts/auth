...
description: Configure and customize the default Auth behavior by createAuth function.
---
...
Supported cookies: `sessionToken`, `csrfToken`, `state`, `code_verifier`, `redirect_to`, and `redirect_uri`. Set per-cookie attributes as needed. For autocomplete and safer suggestions, set the `strategy` field, which provides type-safe defaults that follow cookie best practices.
