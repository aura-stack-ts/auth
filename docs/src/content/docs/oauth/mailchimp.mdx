...
Set up the `Mailchimp` authorization provider in your Aura Auth instance.

---

## What you'll build

Through this quick start guide you are going to learn and understand the basics and how to set up the `Mailchimp` provider for Aura Auth.
...
- [Sign In to Mailchimp (Client & Server)](#sign-in-to-mailchimp-client--server)
- [Resources](#resources)
...
The first step is to create and register an OAuth App on the Mailchimp developer console to obtain access to the user's resources.

1. Log into your Mailchimp account.
2. Navigate to your **Profile > Account > Extras > API keys** (or Registered Apps).
3. Click **Register An App**.
4. Fill in the App Name, Description, and Company Name.
5. In the **Redirect URI** field, enter `http://localhost:3000/auth/callback/mailchimp`.
    - *(Make sure to replace `localhost:3000` with your production domain when deploying)*
6. Click **Create App**. 
7. You will immediately be provided with your **Client ID** and **Client Secret**.
...
Now, you must configure the environment variables required by Aura Auth, including the Mailchimp credentials and the encryption secrets.
...
AURA_AUTH_MAILCHIMP_CLIENT_ID="your_mailchimp_client_id"
AURA_AUTH_MAILCHIMP_CLIENT_SECRET="your_mailchimp_client_secret"
```
...
## Sign In to Mailchimp (Client & Server)
...
`http://localhost:3000/auth/signIn/mailchimp`
...
const triggerSignIn = async () => {
  await authClient.signIn("mailchimp", {
...
export const serverSignIn = async () => {
  const response = await api.signIn("mailchimp", {
...
- [Mailchimp - Authorizing OAuth Apps](https://mailchimp.com/developer/marketing/guides/access-user-data-oauth-2/)
