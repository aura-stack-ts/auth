...
description: Add X authorization provider to Aura Auth to authentication and authorize
---

## X (Twitter)

Set up the `X (Twitter)` authorization provider in your Aura Auth instance.

---

## What you'll build

Through this quick start guide you are going to learn and understand the basics and how to set up the `X` provider for Aura Auth.
...
- [Sign In to X (Client & Server)](#sign-in-to-x-client--server)
- [Resources](#resources)
...
The first step is to create and register an OAuth App on the X developer console to obtain access to the user's resources.

1. Navigate to the **X Developer Portal** (`developer.x.com`).
2. Go to **Projects & Apps > Apps** and click **Create App**.
3. Provide an App Name. Once created, navigate to the App's **Settings**.
4. In the "User authentication settings" section, click **Set up**.
5. Select **Web App** as the App Type and verify **OAuth 2.0** is toggled on.
6. Under App info, input the **Callback URI / Redirect URL**: `http://localhost:3000/auth/callback/x`.
    - *(Make sure to replace `localhost:3000` with your production domain when deploying)*
7. Save the settings to be provided with your **Client ID** and **Client Secret**.
...
Now, you must configure the environment variables required by Aura Auth, including the X credentials and the encryption secrets.
...
# X Credentials
AURA_AUTH_X_CLIENT_ID="your_x_client_id"
AURA_AUTH_X_CLIENT_SECRET="your_x_client_secret"
```
...
`http://localhost:3000/auth/signIn/x`
...
export const serverSignIn = async () => {
  const response = await api.signIn("x", {
...
- [X - Authorizing OAuth Apps](https://developer.x.com/en/docs/authentication/oauth-2-0)
