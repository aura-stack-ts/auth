...
Set up the `Twitch` authorization provider in your Aura Auth instance.

---

## What you'll build

Through this quick start guide you are going to learn and understand the basics and how to set up the `Twitch` provider for Aura Auth.
...
- [Sign In to Twitch (Client & Server)](#sign-in-to-twitch-client--server)
- [Resources](#resources)
...
The first step is to create and register an OAuth App on the Twitch developer console to obtain access to the user's resources.

1. Navigate to the **Twitch Developer Console** (`dev.twitch.tv/console/apps`).
2. Click **Register Your Application**.
3. Enter a Name for your application.
4. In the **OAuth Redirect URLs** field, input `http://localhost:3000/auth/callback/twitch` and click **Add**.
    - *(Make sure to replace `localhost:3000` with your production domain when deploying)*
5. Select a Category (e.g., Website Integration) and verify you aren't a robot.
6. Click **Create**.
7. Click **Manage** on your newly created app to reveal your **Client ID** and generate a **New Secret**.
...
Now, you must configure the environment variables required by Aura Auth, including the Twitch credentials and the encryption secrets.
...
# Twitch Credentials
AURA_AUTH_TWITCH_CLIENT_ID="your_twitch_client_id"
AURA_AUTH_TWITCH_CLIENT_SECRET="your_twitch_client_secret"
```
...
## Sign In to Twitch (Client & Server)
...
`http://localhost:3000/auth/signIn/twitch`
...
const triggerSignIn = async () => {
  await authClient.signIn("twitch", {
...
export const serverSignIn = async () => {
  const response = await api.signIn("twitch", {
...
- [Twitch - Authorizing OAuth Apps](https://dev.twitch.com/docs/authentication)
