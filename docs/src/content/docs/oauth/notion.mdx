...
description: Add Notion authorization provider to Aura Auth to authentication and authorize
---

## Notion

Set up the `Notion` authorization provider in your Aura Auth instance.

---

## What you'll build

Through this quick start guide you are going to learn and understand the basics and how to set up the `Notion` provider for Aura Auth.
...
- [Sign In to Notion (Client & Server)](#sign-in-to-notion-client--server)
- [Resources](#resources)
...
The first step is to create and register an OAuth App on the Notion developer console to obtain access to the user's resources.

1. Navigate to the **Notion My integrations** dashboard.
2. Click **New integration**.
3. Give your integration a name, select an associated workspace, and optionally upload a logo.
4. Scroll down to the **OAuth Domain & URIs** section.
5. Add your **Redirect URI**: `http://localhost:3000/auth/callback/notion`.
    - *(Make sure to replace `localhost:3000` with your production domain when deploying)*
6. Ensure the integration type is set to **Public** to utilize OAuth.
7. Save changes. Under the **Secrets** tab, you will find your **OAuth client ID** and **OAuth client secret**.
...
Now, you must configure the environment variables required by Aura Auth, including the Notion credentials and the encryption secrets.
...
# Notion Credentials
AURA_AUTH_NOTION_CLIENT_ID="your_notion_client_id"
AURA_AUTH_NOTION_CLIENT_SECRET="your_notion_client_secret"
```
...
## Sign In to Notion (Client & Server)
...
`http://localhost:3000/auth/signIn/notion`
...
const triggerSignIn = async () => {
  await authClient.signIn("notion", {
...
export const serverSignIn = async () => {
  const response = await api.signIn("notion", {
...
- [Notion - Authorizing OAuth Apps](https://developers.notion.com/docs/authorization)
