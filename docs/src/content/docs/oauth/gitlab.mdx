...
description: Add GitLab authorization provider to Aura Auth to authentication and authorize
---

## GitLab

Set up the `GitLab` authorization provider in your Aura Auth instance.

---

## What you'll build

Through this quick start guide you are going to learn and understand the basics and how to set up the `GitLab` provider for Aura Auth.
...
- [Sign In to GitLab (Client & Server)](#sign-in-to-gitlab-client--server)
- [Resources](#resources)
...
The first step is to create and register an OAuth App on the GitLab developer console to obtain access to the user's resources.

1. Log into GitLab and navigate to your **User Settings > Applications**.
2. Click **Add new application**.
3. Fill in the "Name" for your application.
4. Set the **Redirect URI** to `http://localhost:3000/auth/callback/gitlab`.
   - _(Make sure to replace `localhost:3000` with your production domain when deploying)_
5. Ensure the **Confidential** checkbox is selected.
6. Select the necessary scopes (e.g., `read_user`).
7. Click **Save application** to generate your **Application ID** (Client ID) and **Secret** (Client Secret).
...
Now, you must configure the environment variables required by Aura Auth, including the GitLab credentials and the encryption secrets.
...
# GitLab Credentials
AURA_AUTH_GITLAB_CLIENT_ID="your_gitlab_client_id"
AURA_AUTH_GITLAB_CLIENT_SECRET="your_gitlab_client_secret"
```
...
## Sign In to GitLab (Client & Server)
...
`http://localhost:3000/auth/signIn/gitlab`
...
const triggerSignIn = async () => {
  await authClient.signIn("gitlab", {
...
export const serverSignIn = async () => {
  const response = await api.signIn("gitlab", {
...
- [GitLab - Authorizing OAuth Apps](https://docs.gitlab.com/ee/integration/oauth_provider.html)
