...
Set the `oauth` option of the `createAuth` instance. You can use the provider name as a string to use default settings, or use the `spotify` factory function to customize the configuration.
...
export const auth = createAuth({
  oauth: [
    spotify({
      scope: "user-read-private user-read-email playlist-read-private", // Custom scopes
    }),
  ],
})
