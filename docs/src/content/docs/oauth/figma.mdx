...
description: Add Figma authorization provider to Aura Auth to authentication and authorize
---

## Figma

Set up the `Figma` authorization provider in your Aura Auth instance.

---

## What you'll build

Through this quick start guide you are going to learn and understand the basics and how to set up the `Figma` provider for Aura Auth.
...
- [Sign In to Figma (Client & Server)](#sign-in-to-figma-client--server)
- [Resources](#resources)
...
The first step is to create and register an OAuth App on the Figma developer console to obtain access to the user's resources.

1. Navigate to your Figma account settings and go to the **Developer Dashboard > Settings > Personal Access Tokens & OAuth**. Or use the Developer Hub to create a new app.
2. Click to **Create a new app** and ensure you select **OAuth 2.0**.
3. Fill out the application details.
4. Find the **Callback URI** or **Redirect URI** section and add `http://localhost:3000/auth/callback/figma`.
    - *(Make sure to replace `localhost:3000` with your production domain when deploying)*
5. Save your app to be issued a **Client ID** and **Client Secret**.
...
Now, you must configure the environment variables required by Aura Auth, including the Figma credentials and the encryption secrets.
...
# Figma Credentials
AURA_AUTH_FIGMA_CLIENT_ID="your_figma_client_id"
AURA_AUTH_FIGMA_CLIENT_SECRET="your_figma_client_secret"
```
...
## Sign In to Figma (Client & Server)
...
`http://localhost:3000/auth/signIn/figma`
...
export const serverSignIn = async () => {
  const response = await api.signIn("figma", {
...
- [Figma - Authorizing OAuth Apps](https://www.figma.com/developers/api)
