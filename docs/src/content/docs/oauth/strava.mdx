...
Set the `oauth` option of the `createAuth` instance. You can use the provider name as a string to use default settings, or use the `strava` factory function to customize the configuration.
...
export const auth = createAuth({
  oauth: [
    strava({
      scope: "read,activity:read_all", // Custom scopes
    }),
  ],
})
